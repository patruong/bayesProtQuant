...
############
'''  
def findUniqueProteins(proteins):
    """
    Take a list of proteins and find and concatenate shared isoform identifications.
    """
    uniqueProteins = []
    for i in proteins:
        protein = i.split("-")[0]
        if (protein not in uniqueProteins) and (protein+"_sharedIsoform" not in uniqueProteins):
            uniqueProteins.append(protein)
        elif protein in uniqueProteins:
            uniqueProteins.remove(protein)
            uniqueProteins.append(protein+"_sharedIsoform")
    return uniqueProteins
'''

#uniprot = getUniProt("library/LKaell/uniprot_sprot_2017-10-25_HUMAN_ISOFORMS.fasta")

# TODO
# STEO 1 READ IN proteins.tsv FIND ALL UNIQUE PROTEINS.
...
"""
proteins = "V6CIS4;V6CIS8;V6CJX7;V6CLC0".split(";")
...
chunked_proteins = []
for i in proteins:
    chunked_proteins.append(textwrap.wrap(i, chunkSize))
"""



#areProteinAccessionSimilar(prot, uniprot, "full")        
#areProteinAccessionSimilar(nonShared[0], uniprot, "full")     
#areProteinAccessionSimilar(prot, uniprot, 10)     
...
# STEP 2 CHECK HOW MANY PROTEINS AND IF ALL OF THEM HAVE A MATCH IN UNIPROT
"""
df["uniqueProteinAccesions"] = df["proteinAccession"].apply(lambda x: len(x.split(";")))
################
proteinAccessions = "A0A0K3ARY0;Q9BKZ9;Q9BKZ9-2;Q9BKZ9-3"
proteinAccessions = "Q9BKZ9;Q9BKZ9-2;Q9BKZ9-3"
proteinAccessions = "Q9BKZ9-2"
proteinAccessions = "Q9BKZ9;Q9BKZ9-2;Q9BKZ9-3" # --> Q9BKZ9_shared
proteinAccessions = "Q9BKZ9" # --> Q9BKZ9
proteinAccessions = "Q9BKZ9-2" # --> Q9BKZ9-2
proteins = proteinAccessions.split(";")
uniqueProteins = findUniqueProteins(proteins)
